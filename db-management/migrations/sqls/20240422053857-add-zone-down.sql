ALTER TABLE content.dataset DROP COLUMN IF EXISTS zone_info; 

-- Drop the zone_extract_w_id function if it exists
DROP FUNCTION IF EXISTS content.zone_extract_w_id(json_data JSON);

-- Drop the content.zone table if it exists
DROP TABLE IF EXISTS content.zone;
