...
--
-- MINIMAL DIFF against the production function. Every edit is marked -- CHANGED.
-- Structure, table names, index names, comments and the entire XML-building and
-- id-remapping half are untouched.
--
-- Edits, in order of appearance:
--   1. NEW temp_node_by_key / temp_node_by_coord  - two small PK'd lookup tables
--      built once from temp_parsed_nodes. The PK is what removes the fan-out
--      risk: at most one match per probe, structurally.
--   2. temp_edge_points  - three columns added (u_id, v_id, n_coords) in the
--      existing INSERT, no extra pass over the data.
--   3. temp_parsed_edge_points - endpoints resolve by identity
--      (edge _u_id/_v_id vs node _id), interior vertices by exact coordinate,
--      otherwise mint. Replaces the ROUND(...,7) join.
--   4. temp_single_ring_polygons / temp_multi_ring_polygons and the zone
--      equivalents - two columns added (ring_len, mint_index) so a ring's
--      closing vertex reuses the first vertex's id. No window functions.
--   5. temp_parsed_zone_polygons - ST_DWithin replaced by the coord lookup.
--   6. temp_all_nodes - the 7 dp CASE/ROUND trimming removed.
--   7. temp_deduplicated_nodes - DISTINCT ON given an ORDER BY so the tagged
--      row always wins.
--
-- Deliberately NOT changed, though both are real improvements, so they can be
-- reviewed separately:
--   * The GIST indexes on the intermediate geom columns are retained even
--     though nothing does spatial predicates any more. They are now pure build
--     cost and can be dropped as an isolated follow-up.
--   * The two DO blocks that remap refs by looping and string-replacing remain
--     as-is. They are the dominant cost on large datasets, but rewriting them
--     means restructuring the whole back half of the function.
...
BEGIN
    ...
    -- CHANGED (1): two lookup tables built once from temp_parsed_nodes.
    -- Both are PRIMARY KEY'd, so a join against either can return at most one
    -- row per probe. That is what makes the vertex resolution below immune to
    -- the fan-out that a tolerance join could produce.
    operation_start_time := clock_timestamp();
    CREATE TEMPORARY TABLE temp_node_by_key (
        id_key VARCHAR PRIMARY KEY,
        node_id VARCHAR
    ) ON COMMIT DROP;
    INSERT INTO temp_node_by_key
    SELECT DISTINCT ON (k) k, node_id
    FROM (
        SELECT COALESCE(feature_json->'properties'->>'_id', node_id) AS k, node_id
        FROM temp_parsed_nodes
    ) s
    ORDER BY k, node_id;

    CREATE TEMPORARY TABLE temp_node_by_coord (
        lat NUMERIC,
        lon NUMERIC,
        node_id VARCHAR,
        PRIMARY KEY (lat, lon)
    ) ON COMMIT DROP;
    INSERT INTO temp_node_by_coord
    SELECT DISTINCT ON (lat, lon) lat, lon, node_id
    FROM temp_parsed_nodes
    ORDER BY lat, lon, node_id;

    ANALYZE temp_node_by_key;
    ANALYZE temp_node_by_coord;
    RAISE NOTICE 'processing node lookups() {%}', clock_timestamp() - operation_start_time;
    ...
    -- CHANGED (2): u_id, v_id and n_coords carried through from the edge feature
    -- in the same pass. n_coords lets the endpoints be identified without a
    -- window function: WITH ORDINALITY numbers 1..N, so the first vertex is
    -- always 1 and the last is always n_coords.
    ...
    SELECT
        ...
        el.feature_json->'properties'->>'_u_id' AS u_id,
        el.feature_json->'properties'->>'_v_id' AS v_id,
        jsonb_array_length(el.feature_json #> '{geometry,coordinates}') AS n_coords
    ...
    CREATE TEMPORARY TABLE temp_parsed_edge_points (
        ...
        is_new BOOLEAN,
        match_src TEXT      -- CHANGED (3): diagnostic - 'uv' | 'coord' | 'new'
    ) ON COMMIT DROP;
    -- CHANGED (3): resolution order per vertex is now
    --   a. ENDPOINTS - identity: edge properties._u_id (first vertex) or ._v_id
    --      (last vertex) against node properties._id. Resolves topology by
    --      identity, so two nodes sharing a coordinate each attach to the right
    --      edge, and float representation never enters into it.
    --   b. INTERIOR VERTICES, and endpoints whose _u_id/_v_id does not resolve -
    --      exact coordinate equality. Replaces ROUND(...,7) = ROUND(...,7);
    --      coordinates are assumed to be within precision.
    --   c. Neither matched - mint a new node id, as before.
    -- Both joins target PK'd lookup tables, so neither can duplicate a vertex.
    ...
    SELECT 
        ...
        COALESCE(
            k.node_id,
            c.node_id,
			(ep.edge_id || '_' || ep.point_index )::VARCHAR
        ) AS final_node_id,
        COALESCE(k.node_id, c.node_id) IS NULL AS is_new,
        CASE WHEN k.node_id IS NOT NULL THEN 'uv'
             WHEN c.node_id IS NOT NULL THEN 'coord'
             ELSE 'new' END AS match_src
    FROM temp_edge_points ep
    LEFT JOIN temp_node_by_key k
        ON k.id_key = CASE WHEN ep.point_index = 1            THEN ep.u_id
                           WHEN ep.point_index = ep.n_coords  THEN ep.v_id
                      END
    LEFT JOIN temp_node_by_coord c
        ON c.lat = ep.lat::NUMERIC AND c.lon = ep.lon::NUMERIC;
    ...
    -- CHANGED (3): resolution breakdown. Endpoints reported as 'coord' rather
    -- than 'uv' mean _u_id/_v_id values that do not match any node _id in this
    -- dataset - worth investigating before trusting the output.
    DECLARE
        c_uv BIGINT; c_coord BIGINT; c_new BIGINT;
    BEGIN
        SELECT count(*) FILTER (WHERE match_src = 'uv'),
               count(*) FILTER (WHERE match_src = 'coord'),
               count(*) FILTER (WHERE match_src = 'new')
        INTO c_uv, c_coord, c_new
        FROM temp_parsed_edge_points;
        RAISE NOTICE 'edge vertex resolution: by _u_id/_v_id=%, by coordinate=%, minted=%',
            c_uv, c_coord, c_new;
    END;
    ...
    SELECT 
        true AS is_new,
		('point_' || pp.point_id) AS final_node_id,
        pp.lat,
        pp.lon,
        jsonb_build_object(
            'type', 'Feature',
    ...
    SELECT
        ep.*,
		(ep.line_id || '_' || ep.point_index)::VARCHAR AS final_node_id,
        true AS is_new
    ...
    -- CHANGED (4): mint_index added. A GeoJSON ring repeats its first coordinate
    -- as its last, so the closing vertex reuses index 1 and the emitted way ends
    -- on the same <nd ref> it started with. Previously first and last minted
    -- different ids and every ring was an unclosed way, which OSM rejects for an
    -- area. Computed inline from the ring already in scope - no window function.
    ...
    SELECT 
        ...
        CASE WHEN point_idx > 1
              AND point_idx = jsonb_array_length(pf.feature_json #> '{geometry,coordinates,0}')
              AND (coord->>0)::NUMERIC = (pf.feature_json #>> '{geometry,coordinates,0,0,0}')::NUMERIC
              AND (coord->>1)::NUMERIC = (pf.feature_json #>> '{geometry,coordinates,0,0,1}')::NUMERIC
             THEN 1 ELSE point_idx END AS mint_index
    ...
    -- CHANGED (4): mint_index, same rationale. Here the ring array is already in
    -- scope as ring_coords, so the lookups are direct.
    ...
    SELECT 
        ...
        ST_SetSRID(ST_MakePoint((coord->>0)::DOUBLE PRECISION, (coord->>1)::DOUBLE PRECISION), 4326) AS geom,
        CASE WHEN point_idx > 1
              AND point_idx = jsonb_array_length(ring_coords)
              AND (coord->>0)::NUMERIC = (ring_coords #>> '{0,0}')::NUMERIC
              AND (coord->>1)::NUMERIC = (ring_coords #>> '{0,1}')::NUMERIC
             THEN 1 ELSE point_idx END AS mint_index
    FROM temp_extension_polygons pf
    ...
    SELECT 
        ...
        fc.is_multipolygon,
        fc.geom,
        -- CHANGED (4): mint_index instead of point_index closes the ring
		(fc.polygon_id || '-' || fc.mint_index || '-' || fc.ring_index || '-' || fc.is_multipolygon)::VARCHAR AS final_node_id,
        true AS is_new
    ...
    -- CHANGED (4): mint_index, same as polygons.
    ...
    SELECT 
        ...
        ST_SetSRID(ST_MakePoint((coord->>0)::DOUBLE PRECISION, (coord->>1)::DOUBLE PRECISION), 4326) AS geom,
        CASE WHEN point_idx > 1
              AND point_idx = jsonb_array_length(z.feature::jsonb #> '{geometry,coordinates,0}')
              AND (coord->>0)::NUMERIC = (z.feature::jsonb #>> '{geometry,coordinates,0,0,0}')::NUMERIC
              AND (coord->>1)::NUMERIC = (z.feature::jsonb #>> '{geometry,coordinates,0,0,1}')::NUMERIC
             THEN 1 ELSE point_idx END AS mint_index
    FROM content.zone z
    ...
    -- CHANGED (5): ST_DWithin(..., 1e-9) replaced by the PK'd coordinate lookup.
    -- Zones carry _w_id, which identifies the way rather than per-vertex nodes,
    -- so there is no identity signal here - coordinates remain the only option.
    -- CHANGED (4): mint_index closes the ring on the minted branch.
    ...
    SELECT 
		...
		COALESCE(
            c.node_id, 
			(fc.zone_id || '_' || fc.mint_index || '_' || fc.ring_index || '_' || fc.is_multipolygon)::VARCHAR 
        ) AS final_node_id,
        c.node_id IS NULL AS is_new
    FROM temp_flattened_zone_coords fc
    LEFT JOIN temp_node_by_coord c
        ON c.lat = fc.lat::NUMERIC AND c.lon = fc.lon::NUMERIC;
    -- Index on zone_id for grouping
    -- Reason: Optimizes grouping by zone_id for way creation
    ...
    -- CHANGED (6): the 7 dp CASE/ROUND trimming is removed from all twelve
    -- lat/lon expressions. Coordinates are assumed to be within precision and
    -- are emitted as supplied.
    ...
    SELECT node_id, lat, lon, feature_json
    FROM temp_parsed_nodes
    UNION ALL
    SELECT final_node_id AS node_id, lat, lon, feature_json
    FROM temp_parsed_extension_points
    UNION ALL
    SELECT final_node_id AS node_id, lat::NUMERIC, lon::NUMERIC, NULL::JSONB AS feature_json
    ...
    -- Purpose: Ensures each node_id appears exactly once. Two distinct node_ids
    --          MAY share identical coordinates - that is expected and no longer
    --          collapsed.
    ...
    -- CHANGED (7): DISTINCT ON had no ORDER BY, so when a real node id was also
    -- referenced by an edge or zone vertex (feature_json NULL) the surviving row
    -- was arbitrary and the tags could be silently lost. The ordering guarantees
    -- the tagged row wins.
    ...
    SELECT DISTINCT ON (node_id)
        ...
        feature_json
    FROM temp_all_nodes
    ORDER BY node_id, (feature_json IS NULL);
    -- Index on node_id for lookups
    -- Reason: Optimizes references to node IDs in OSM output
    ...
    SELECT
		('edge_' || pep.edge_id) as eid,
		ARRAY_AGG(final_node_id) as node_refs,
        '<way visible="true"  version="1" id="' || 'edge_' || pep.edge_id || '">' ||
        STRING_AGG('<nd ref="' || final_node_id || '"/>', E'' ORDER BY point_index) ||
        COALESCE(
            (
                SELECT STRING_AGG('<tag k="' || Key || '" v="' || CASE 
                        WHEN key = 'width' THEN TO_CHAR(value::float8, 'FM999999990.0999999')::text
    ...
    SELECT
       	(n'polygon_rel_' || pep.polygon_id) as eid,
		  ARRAY_AGG(pep.polygon_id || '-' || ring_index || '-' || is_multipolygon) as way_refs,
		 '<relation visible="true"  version="1" id="' || 'polygon_rel_' || pep.polygon_id || '">' || 
        STRING_AGG(
            DISTINCT 
    ...
    SELECT
        ring_index,
		(pzp.zone_id || '_' || ring_index || '_' || is_multipolygon) as eid,
       	ARRAY_AGG(final_node_id) as node_refs,
		 '<way visible="true"  version="1" id="' || (pzp.zone_id || '_' || ring_index || '_' || is_multipolygon)  || '">' ||
        STRING_AGG('<nd ref="' || final_node_id || '"/>', E'' ORDER BY point_index) ||
        COALESCE(
            (
                SELECT STRING_AGG('<tag k="' || Key || '" v="' || CASE 
    ...
    CREATE INDEX idx_temp_zone_way_blocks_eid ON temp_zone_way_blocks(eid);
    ...
    SELECT
		('zone_rel_'|| pzp.zone_id) as eid,
		ARRAY_AGG(pzp.zone_id || '_' || ring_index || '_' || is_multipolygon) as way_refs,
        '<relation visible="true"  version="1" id="' || 'zone_rel_'|| pzp.zone_id || '">' || 
        STRING_AGG(
            DISTINCT 
            '<member type="way" ref="' || (pzp.zone_id || '_' || ring_index || '_' || is_multipolygon)  || 
            '" role="' || CASE WHEN ring_index = 1 THEN 'outer' ELSE 'inner' END || '"/>',
            ''
        ) ||
        '<tag k="type" v="multipolygon"/>' || 
    ...
    SELECT
		('line_' || pel.line_id) as eid,
		ARRAY_AGG(final_node_id) as node_refs,
        '<way visible="true"  version="1" id="' || 'line_' || pel.line_id || '">' ||
        STRING_AGG('<nd ref="' || final_node_id || '"/>', E'' ORDER BY point_index) ||
        COALESCE(
            (
                SELECT STRING_AGG('<tag k="' || Key || '" v="' || CASE 
