...
BEGIN
	...
	ORDER by edge_id ASC;
	 ...
	 CREATE TEMP TABLE temp_intersected_nodes (
        node_id character varying
    ) ON COMMIT DROP;
	
    IF NOT EXISTS (SELECT 1 FROM temp_intersected_edges) THEN
		INSERT INTO temp_intersected_nodes(node_id)
        SELECT n.node_id
        FROM content.node n
        WHERE n.tdei_dataset_id = dataset_id
          AND ST_Intersects(
                n.node_loc,
                ST_MakeEnvelope(x_min_param, y_min_param, x_max_param, y_max_param, 4326)
              );
    END IF;
...
--     -- Iterate over intersected nodes
    ...
    OPEN result_cursor FOR 
        ...
        JOIN (
			...
			UNION
			SELECT node_id FROM temp_intersected_nodes
