  ...
  END AS lon, NULL::JSONB AS feature_json
    ...
    SELECT
        CASE 
            ...
            THEN '<node visible="true" id="' || abs(hashtext(node_id))::bigint || '" lat="' || lat || '" lon="' || lon || '">' || (
                SELECT string_agg('<tag k="' || key || '" v="' || content.tdei_escape_xml_attr(value) || '"/>', E'')
                FROM jsonb_each_text(feature_json->'properties')
                WHERE key NOT IN ('_id', '_u_id', '_v_id', '_w_id')
            ) || '</node>'
            ELSE '<node visible="true" id="' || abs(hashtext(node_id))::bigint || '" lat="' || lat || '" lon="' || lon || '"/>'
        END AS line,
    ...
    SELECT
        ...
        '<way visible="true" id="' || abs(hashtext(pep.edge_id))::bigint || '">' ||
    ...
    SELECT
        ...
        '<way visible="true" id="' || abs(hashtext(pep.polygon_id || ring_index::text))::bigint  || '">' ||
        STRING_AGG('<nd ref="' || abs(hashtext(final_node_id))::bigint || '"/>', E'' ORDER BY point_index) ||
        COALESCE(
            (
                SELECT STRING_AGG('<tag k="' || Key || '" v="' || CASE 
    ...
    SELECT
        pep.polygon_id,
		 ...
		 '<relation visible="true" id="' || abs(hashtext(pep.polygon_id))::bigint || '">' || 
        STRING_AGG(
            DISTINCT 
    ...
    SELECT
        ...
        '<way visible="true" id="' || abs(hashtext(pzp.zone_id || COALESCE(ring_index::text, '0')))::bigint  || '">' ||
        STRING_AGG('<nd ref="' || abs(hashtext(final_node_id))::bigint || '"/>', E'' ORDER BY point_index) ||
        COALESCE(
            (
                SELECT STRING_AGG('<tag k="' || Key || '" v="' || CASE 
    ...
    SELECT
        ...
        '<relation visible="true" id="' || abs(hashtext(pzp.zone_id))::bigint || '">' || 
        STRING_AGG(
            DISTINCT 
    ...
    SELECT
        ...
        '<way visible="true" id="' || abs(hashtext(pel.line_id))::bigint || '">' ||
        STRING_AGG('<nd ref="' || abs(hashtext(final_node_id))::bigint || '"/>', E'' ORDER BY point_index) ||
        COALESCE(
            (
                SELECT STRING_AGG('<tag k="' || Key || '" v="' || CASE 
	...
	CREATE TEMPORARY TABLE temp_way_blocks (
    line TEXT,
    seq_id BIGINT
	) ON COMMIT DROP;
	INSERT INTO temp_way_blocks (line, seq_id)
	SELECT line, seq_id FROM (
	    SELECT line, seq_id FROM temp_edge_way_blocks
	    UNION ALL
	    SELECT line, seq_id FROM temp_polygon_way_blocks
	    UNION ALL
	    SELECT line, seq_id FROM temp_zone_way_blocks
	    UNION ALL
	    SELECT line, seq_id FROM temp_extension_lines_way_blocks
	    UNION ALL
	    SELECT line, seq_id FROM temp_polygon_relation_blocks
	) AS combined_ways
	ORDER BY seq_id ASC;
		...
		-- relations
		INSERT INTO temp_exportdata
		SELECT line,seq_id, 3 as type FROM temp_zone_relation_blocks ORDER BY seq_id asc;
