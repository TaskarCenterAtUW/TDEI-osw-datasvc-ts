...
-- CREATE OR REPLACE does not replace a function whose argument list differs, it
-- adds an overload. Both would then match a 3-argument call and PostgreSQL
-- rejects it as "function ... is not unique", so drop the old signature first.
...
CREATE OR REPLACE FUNCTION content.tdei_union_dataset(
    ...
    -- Optional per-file-type attribute filters controlling which features are
    -- ELIGIBLE TO MERGE. Nothing is ever removed from the output by a filter:
    -- a DS1 feature failing it passes through unchanged, a DS2 feature failing
    -- it is added as new. NULL (the default) = current behaviour everywhere.
    --
    --   {"edge":    {"filters": [ {"highway":"footway","footway":"sidewalk"},
    --                             {"highway":"footway","footway":"crossing"} ]},
    --    "line":    {"filters": [ {"highway":"path"} ]}}
    --
    -- Within a group the key/value pairs are AND-ed; across groups they are
    -- OR-ed. Implemented with the jsonb containment operator @>, whose
    -- semantics are exactly "properties contain all pairs in this group".
    --
    -- Duplicate-detection settings live in the same blocks:
    --   duplicate_buffer_width        metres   — edge, line
    --   duplicate_overlap_percentage  0-100    — edge, line, polygon, zone
    --   {"edge": {"duplicate_buffer_width": 3, "duplicate_overlap_percentage": 70}, "polygon": {"duplicate_overlap_percentage": 80}}
    -- Defaults: buffer = proximity; overlap 80% edges, 70% lines/polygons/zones.
    -- Unknown file types or keys, and settings on a file type that cannot use
    -- them, are rejected with an error rather than silently ignored.
...
DECLARE
    ...
    keep_added              INTEGER;          -- stranding-guard loop counter
    -- Defaults are the values in use before these became configurable, so an
    -- unconfigured run is unchanged.
    edge_overlap            FLOAT8 := 0.80;
    line_overlap            FLOAT8 := 0.70;
    polygon_overlap         FLOAT8 := 0.70;
    zone_overlap            FLOAT8 := 0.70;
    edge_buffer_m           FLOAT8;           -- metres; defaults to proximity
    line_buffer_m           FLOAT8;
    edge_buffer_deg         FLOAT8;           -- corridor radius in degrees
    line_buffer_deg         FLOAT8;
    cfg_type                TEXT;             -- config validation loop vars
    cfg                     JSONB;
    cfg_key                 TEXT;
    -- Metres per degree. All geometry here is SRID 4326 *geometry* (not
    -- geography), so PostGIS measures distance as planar arithmetic in DEGREES
    -- and does not account for a degree of longitude shortening with latitude.
    -- One degree of latitude is ~111,111 m everywhere; one degree of longitude
    -- is 111,111 * cos(latitude) — only 79,073 m at 44.6°N.
    m_per_deg_lat           FLOAT8 := 111111.0;
    m_per_deg_lon           FLOAT8;   -- derived from the data's centre latitude
    centre_lat              FLOAT8;
    -- Per-file-type filter groups (JSONB array), NULL when no filter supplied.
    f_edge                  JSONB;
    f_node                  JSONB;
    f_zone                  JSONB;
    f_line                  JSONB;
...
BEGIN
    ...
    -- ── Validate the per-file-type configuration ─────────────────────────────
    -- Anything unrecognised is REJECTED, never silently ignored: a mistyped key
    -- or file type that was quietly dropped would produce a run that looks
    -- clean while applying none of the caller's intent.
    IF entity_filters IS NOT NULL THEN
        IF jsonb_typeof(entity_filters) <> 'object' THEN
            RAISE EXCEPTION 'entity_filters must be a JSON object keyed by file type (edge, node, zone, line, polygon, point)';
        END IF;
        FOR cfg_type, cfg IN SELECT key, value FROM jsonb_each(entity_filters) LOOP
            IF cfg_type NOT IN ('edge','node','zone','line','polygon','point') THEN
                RAISE EXCEPTION 'entity_filters: unknown file type "%" — expected edge, node, zone, line, polygon or point', cfg_type;
            END IF;
            IF jsonb_typeof(cfg) <> 'object' THEN
                RAISE EXCEPTION 'entity_filters.% must be an object', cfg_type;
            END IF;
            FOR cfg_key IN SELECT jsonb_object_keys(cfg) LOOP
                IF cfg_key NOT IN ('filters','duplicate_buffer_width','duplicate_overlap_percentage') THEN
                    RAISE EXCEPTION 'entity_filters.%: unknown setting "%" — expected filters, duplicate_buffer_width or duplicate_overlap_percentage', cfg_type, cfg_key;
                END IF;
            END LOOP;
            IF cfg ? 'filters' AND jsonb_typeof(cfg->'filters') <> 'array' THEN
                RAISE EXCEPTION 'entity_filters.%.filters must be a list of filter groups', cfg_type;
            END IF;
            IF cfg ? 'duplicate_buffer_width' THEN
                IF cfg_type IN ('polygon','zone') THEN
                    RAISE EXCEPTION 'entity_filters.%.duplicate_buffer_width is not supported: % features are compared by shared area, so there is no corridor to widen', cfg_type, cfg_type;
                ELSIF cfg_type IN ('node','point') THEN
                    RAISE EXCEPTION 'entity_filters.%.duplicate_buffer_width is not supported: % features are merged by proximity, not by overlap', cfg_type, cfg_type;
                END IF;
                IF jsonb_typeof(cfg->'duplicate_buffer_width') <> 'number' THEN
                    RAISE EXCEPTION 'entity_filters.%.duplicate_buffer_width must be a number of metres, not % (got %)', cfg_type, jsonb_typeof(cfg->'duplicate_buffer_width'), cfg->'duplicate_buffer_width';
                ELSIF (cfg->>'duplicate_buffer_width')::FLOAT8 <= 0 THEN
                    RAISE EXCEPTION 'entity_filters.%.duplicate_buffer_width must be greater than 0 metres (got %)', cfg_type, cfg->'duplicate_buffer_width';
                END IF;
            END IF;
            IF cfg ? 'duplicate_overlap_percentage' THEN
                IF cfg_type IN ('node','point') THEN
                    RAISE EXCEPTION 'entity_filters.%.duplicate_overlap_percentage is not supported: % features are merged by proximity, not by overlap', cfg_type, cfg_type;
                END IF;
                IF jsonb_typeof(cfg->'duplicate_overlap_percentage') <> 'number' THEN
                    RAISE EXCEPTION 'entity_filters.%.duplicate_overlap_percentage must be a number, not % (got %)', cfg_type, jsonb_typeof(cfg->'duplicate_overlap_percentage'), cfg->'duplicate_overlap_percentage';
                ELSIF (cfg->>'duplicate_overlap_percentage')::FLOAT8 <= 0
                   OR (cfg->>'duplicate_overlap_percentage')::FLOAT8 > 100 THEN
                    RAISE EXCEPTION 'entity_filters.%.duplicate_overlap_percentage must be greater than 0 and at most 100 (got %)', cfg_type, cfg->'duplicate_overlap_percentage';
                END IF;
            END IF;
        END LOOP;
    END IF;

    -- Overlap thresholds: supplied percentage, else that file type's own default.
    edge_overlap    := COALESCE((entity_filters #>> '{edge,duplicate_overlap_percentage}')::FLOAT8    / 100.0, edge_overlap);
    line_overlap    := COALESCE((entity_filters #>> '{line,duplicate_overlap_percentage}')::FLOAT8    / 100.0, line_overlap);
    polygon_overlap := COALESCE((entity_filters #>> '{polygon,duplicate_overlap_percentage}')::FLOAT8 / 100.0, polygon_overlap);
    zone_overlap    := COALESCE((entity_filters #>> '{zone,duplicate_overlap_percentage}')::FLOAT8    / 100.0, zone_overlap);

    -- Extract the filter groups for each file type. A missing block, a missing
    -- 'filters' key, or an empty list all mean "no filter" for that file type.
    f_edge    := NULLIF(entity_filters #> '{edge,filters}',    '[]'::jsonb);
    f_node    := NULLIF(entity_filters #> '{node,filters}',    '[]'::jsonb);
    f_zone    := NULLIF(entity_filters #> '{zone,filters}',    '[]'::jsonb);
    f_line    := NULLIF(entity_filters #> '{line,filters}',    '[]'::jsonb);
    f_polygon := NULLIF(entity_filters #> '{polygon,filters}', '[]'::jsonb);
    f_point   := NULLIF(entity_filters #> '{point,filters}',   '[]'::jsonb);

    RAISE NOTICE 'Filters: edge=% node=% zone=% line=% polygon=% point=%',
        COALESCE(jsonb_array_length(f_edge),0),    COALESCE(jsonb_array_length(f_node),0),
        COALESCE(jsonb_array_length(f_zone),0),    COALESCE(jsonb_array_length(f_line),0),
        COALESCE(jsonb_array_length(f_polygon),0), COALESCE(jsonb_array_length(f_point),0);
    ...
    -- ── Latitude-aware proximity scale ──────────────────────────────────────
    -- proximity_degrees is set HERE and only here: it needs the centre latitude,
    -- which is only known once the nodes are loaded. Take that centre latitude
    -- and derive metres-per-degree of longitude there. A single value is accurate for a city-sized extent:
    -- cos(lat) varies by well under 1% across half a degree of latitude.
    --
    -- proximity_degrees is then derived from the LONGITUDE scale, the smaller
    -- of the two metres-per-degree values. That makes the degree radius large
    -- enough to reach the requested distance in EVERY direction: the region is
    -- an ellipse that circumscribes the intended circle, so a true match is
    -- never missed. It reaches further north-south than requested, which is the
    -- safe direction to err — extra candidates are merely considered, and the
    -- nearest is then chosen by TRUE METRIC distance (see node_map), with the
    -- type guard and kerb exception still applying.
    SELECT AVG(ST_Y(geom)) INTO centre_lat
    FROM (SELECT geom FROM ds1_nodes UNION ALL SELECT geom FROM ds2_nodes) allnodes;

    centre_lat        := COALESCE(centre_lat, 0.0);
    m_per_deg_lon     := m_per_deg_lat * COS(RADIANS(centre_lat));
    proximity_degrees := proximity / GREATEST(m_per_deg_lon, 1.0);

    -- Duplicate-detection corridors, INDEPENDENT of proximity: proximity decides
    -- which nodes connect; buffer width decides how far a secondary edge or line
    -- may sit from an authoritative one and still count as the same feature.
    -- Widening the corridor never widens node merging.
    edge_buffer_m   := COALESCE((entity_filters #>> '{edge,duplicate_buffer_width}')::FLOAT8, proximity);
    line_buffer_m   := COALESCE((entity_filters #>> '{line,duplicate_buffer_width}')::FLOAT8, proximity);
    edge_buffer_deg := edge_buffer_m / GREATEST(m_per_deg_lon, 1.0);
    line_buffer_deg := line_buffer_m / GREATEST(m_per_deg_lon, 1.0);

    RAISE NOTICE 'Duplicate detection: edge buffer=% m overlap=%, line buffer=% m overlap=%, polygon overlap=%, zone overlap=%',
        ROUND(edge_buffer_m::NUMERIC,3), ROUND((edge_overlap*100)::NUMERIC,1)::TEXT || '%',
        ROUND(line_buffer_m::NUMERIC,3), ROUND((line_overlap*100)::NUMERIC,1)::TEXT || '%',
        ROUND((polygon_overlap*100)::NUMERIC,1)::TEXT || '%',
        ROUND((zone_overlap*100)::NUMERIC,1)::TEXT || '%';

    RAISE NOTICE 'Proximity scale: centre_lat=%, m/deg_lon=%, proximity=% m -> % deg',
        ROUND(centre_lat::NUMERIC,4), ROUND(m_per_deg_lon::NUMERIC,1),
        proximity, proximity_degrees;
    ...
    SELECT
        ...
        geom,
        feature,
        -- Merge eligibility under the edge filter. Independent of type_group
        -- below: classification always runs on the UNFILTERED dataset, because
        -- an unclassified edge would leave its nodes untyped, and untyped nodes
        -- take the permissive branch of the type guard — filtering would then
        -- LOOSEN merging instead of tightening it.
        (f_edge IS NULL OR EXISTS (
            SELECT 1 FROM jsonb_array_elements(f_edge) g
            WHERE (feature::jsonb->'properties') @> g
        ))                              AS filter_ok,
        CASE
    ...
    CREATE INDEX ON edge_type_groups (filter_ok);
    ...
    SELECT
        n.element_id,
        n.geom,
        -- Type groups: from ALL incident edges (classification is unfiltered).
        COALESCE(
            ARRAY_AGG(DISTINCT etg.type_group)
                FILTER (WHERE etg.type_group IS NOT NULL),
            ARRAY[]::TEXT[]
        ) AS groups,
        -- Merge eligibility: a node may merge when it belongs to an edge that
        -- satisfies the edge filter. With no edge filter every node is eligible.
        -- A node on no edge is eligible only when no filter is in force.
        CASE
            WHEN f_edge IS NULL THEN TRUE
            ELSE COALESCE(bool_or(etg.filter_ok), FALSE)
        END AS filter_ok
    FROM ds1_nodes n
    ...
    -- Filter eligibility report. A filter naming an attribute no feature carries
    -- yields 0 eligible and nothing merges; surfacing the counts makes a
    -- mistyped attribute obvious instead of looking like a clean run.
    IF entity_filters IS NOT NULL THEN
        RAISE NOTICE 'Filter eligibility — edges: %/% eligible',
            (SELECT COUNT(*) FROM edge_type_groups WHERE filter_ok),
            (SELECT COUNT(*) FROM edge_type_groups);
        RAISE NOTICE 'Filter eligibility (attribute merge only; connectivity is never filtered) — ds1 nodes: %/%, ds2 nodes: %/%',
            (SELECT COUNT(*) FROM ds1_node_groups WHERE filter_ok),
            (SELECT COUNT(*) FROM ds1_node_groups),
            (SELECT COUNT(*) FROM ds2_node_groups WHERE filter_ok),
            (SELECT COUNT(*) FROM ds2_node_groups);
    END IF;
    ...
    SELECT DISTINCT ON (n2.element_id)
        ...
        -- true ground distance in METRES (degree-space distance would be
        -- distorted east-west; see the proximity scale note in Phase 1)
        CASE WHEN d1.element_id IS NOT NULL
             THEN SQRT(POWER((ST_X(n2.geom) - ST_X(d1.geom)) * m_per_deg_lon, 2) + POWER((ST_Y(n2.geom) - ST_Y(d1.geom)) * m_per_deg_lat, 2)) END  AS snap_dist,
        ...
        CASE
            ...
            ELSE 0.5
        END                                      AS type_match,
        -- Attribute-merge eligibility. The node ALWAYS connects when it snaps;
        -- this only decides whether its DS2 attributes are merged onto the DS1
        -- node. Both sides must pass the edge-derived eligibility and, if one
        -- was supplied, the explicit node filter. DS2 values are audited either
        -- way, so nothing is silently discarded.
        (d1.element_id IS NOT NULL
         AND COALESCE(g2.filter_ok, TRUE)
         AND COALESCE(g1.filter_ok, TRUE)
         AND (f_node IS NULL OR EXISTS (
                 SELECT 1 FROM jsonb_array_elements(f_node) g
                 WHERE (n2.feature::jsonb->'properties') @> g))
         AND (f_node IS NULL OR EXISTS (
                 SELECT 1 FROM jsonb_array_elements(f_node) g
                 WHERE (d1.feature::jsonb->'properties') @> g))
        )                                        AS attr_merge_ok
    FROM ds2_nodes n2
    ...
    LEFT JOIN LATERAL (
        ...
        -- Two-step distance test.
        --   1. ST_DWithin with the degree radius: GIST-indexed prefilter. The
        --      radius is derived from the longitude scale so the region always
        --      CIRCUMSCRIBES the intended circle — it never misses a true match,
        --      but it reaches ~1/cos(lat) too far north-south.
        --   2. exact metric distance <= proximity: trims that ellipse back to a
        --      true circle of exactly `proximity` METRES, so "within 1 m" means
        --      1 m in every direction, at any latitude.
        -- The prefilter keeps the index usable; the exact test keeps it correct.
        WHERE ST_DWithin(n2.geom, d1.geom, proximity_degrees)
          AND SQRT(POWER((ST_X(n2.geom) - ST_X(d1.geom)) * m_per_deg_lon, 2) + POWER((ST_Y(n2.geom) - ST_Y(d1.geom)) * m_per_deg_lat, 2)) <= proximity
          ...
          -- COINCIDENCE OVERRIDE, then the type/kerb heuristics.
          --
          -- Two nodes at the SAME coordinate are the same physical point, so
          -- they must always merge. The type guard and the kerb exception exist
          -- to keep DISTINCT nearby features apart; applying them at distance
          -- zero splits a point from itself and leaves the network
          -- topologically broken — e.g. a crossing ending exactly on a road
          -- node, or the OSW sidewalk → plain footway → crossing chain, would
          -- end up as two ids at one location with no connection between them.
          AND (
                ST_Equals(n2.geom, d1.geom)     -- same point → always merge
             OR (
                  -- NOTE the COALESCE: ->>'barrier' is NULL on a node with no
                  -- barrier key, and NULL = 'kerb' is NULL (not FALSE). Without
                  -- it the predicate is NULL for ordinary nodes, a WHERE treats
                  -- NULL as "not true", and EVERY candidate is rejected —
                  -- silently disabling node merging altogether.
                  NOT (
                        COALESCE((n2.feature::jsonb->'properties'->>'barrier') = 'kerb', FALSE)
                    AND COALESCE((d1.feature::jsonb->'properties'->>'barrier') = 'kerb', FALSE)
                  )
                  AND (
          ...
          -- ATTRIBUTE FILTER: both sides of the pair must be eligible. Placed
          -- here in the candidate LATERAL, never in an outer WHERE: a WHERE
          -- would drop the DS2 node itself and destroy its edges. An ineligible
          -- node simply finds no candidate, stays unsnapped, and is kept.
          -- ROUTABILITY PRINCIPLE: attribute filters are deliberately NOT part
          -- of this decision. In OSW two edges are connected only when they
          -- share a node id, so snapping IS connectivity; any filter that
          -- blocked a snap here would disconnect the network at that point
          -- (and, where the DS2 edge is later removed as a duplicate, strand
          -- whatever was attached to it). Filters instead govern attribute
          -- merging (attr_merge_ok, below) and duplicate removal of edges.
        -- nearest by TRUE metres, not degree-space distance
        ORDER BY SQRT(POWER((ST_X(n2.geom) - ST_X(d1.geom)) * m_per_deg_lon, 2) + POWER((ST_Y(n2.geom) - ST_Y(d1.geom)) * m_per_deg_lat, 2)) ASC
        LIMIT 1
    ) d1 ON TRUE
    LEFT JOIN ds1_node_groups g1 ON g1.element_id = d1.element_id
    ORDER BY
        n2.element_id,
        SQRT(POWER((ST_X(n2.geom) - ST_X(d1.geom)) * m_per_deg_lon, 2) + POWER((ST_Y(n2.geom) - ST_Y(d1.geom)) * m_per_deg_lat, 2)) ASC NULLS LAST;
    ...
    FROM (
        ...
        FROM ds2_edges e
        JOIN ds1_nodes n
            -- indexed prefilter, then exact metric distance to the nearest point
            -- ON the line, so "within proximity" is a true circle in metres
            ON ST_DWithin(e.geom, n.geom, proximity_degrees)
           AND SQRT(POWER((ST_X(ST_ClosestPoint(e.geom, n.geom)) - ST_X(n.geom)) * m_per_deg_lon, 2)
                  + POWER((ST_Y(ST_ClosestPoint(e.geom, n.geom)) - ST_Y(n.geom)) * m_per_deg_lat, 2))
               <= proximity
        -- DS2 edge type group
    ...
    -- ── T-JUNCTIONS ONTO A DS1 SPAN (routability) ─────────────────────────────
    -- OSW connects edges only through shared nodes, and a DS1 edge is joined only
    -- at its endpoints. A DS2 edge that ends on the MIDDLE of a DS1 edge therefore
    -- never connects: DS1 nodes split DS2 edges (Phase 3 above), but nothing split
    -- a DS1 edge at a DS2 junction. Without this, such spurs are islands, and a
    -- DS2 duplicate running mid-span strands whatever is attached to it once it is
    -- removed as a duplicate.
    --
    -- Routability outranks DS1 immutability, so the DS1 edge is split at the
    -- junction — the same mechanism road×crossing uses. The DS1 shape is kept:
    -- the split point is placed ON the DS1 line (snapped to the 7dp grid) and both
    -- sides are forced to it.
    --
    -- Scope, to keep DS1 changes to the minimum routability requires:
    --   * never road edges — road geometry and topology are authoritative;
    --   * only type-compatible pairs (same guard as node snapping);
    --   * only where a DS2 edge meets the DS1 edge at an ANGLE (>= 30 deg). A DS2
    --     edge running ALONGSIDE is a duplicate, not a junction; splitting DS1 at
    --     every vertex of a parallel duplicate would fragment it for no gain.
    -- Two situations:
    --   (a) the DS2 node had nothing to snap to, but lies within proximity of the
    --       DS1 edge's interior -> split at the nearest point on the DS1 line and
    --       move the DS2 node there;
    --   (b) the DS2 node snapped to a DS1 node that is only an interior VERTEX of a
    --       DS1 edge (not an endpoint of any DS1 edge), so it joins no DS1 edge ->
    --       split that DS1 edge at the vertex (exact, no new coordinates).

    -- Direction of each DS2 edge leaving each of its endpoints.
    DROP TABLE IF EXISTS ds2_endpoint_dirs;
    CREATE TEMP TABLE ds2_endpoint_dirs ON COMMIT DROP AS
    -- pt = the endpoint, nxt = next vertex along the edge, far = opposite endpoint
    SELECT ST_StartPoint(geom) AS pt, ST_PointN(geom, 2) AS nxt, ST_EndPoint(geom) AS far FROM ds2_edges
    UNION ALL
    SELECT ST_EndPoint(geom),  ST_PointN(geom, ST_NPoints(geom) - 1), ST_StartPoint(geom) FROM ds2_edges;
    CREATE INDEX ON ds2_endpoint_dirs USING GIST (pt);

    -- DS1 edge endpoints, to tell a genuine DS1 junction from an interior vertex.
    DROP TABLE IF EXISTS ds1_edge_ends;
    CREATE TEMP TABLE ds1_edge_ends ON COMMIT DROP AS
    SELECT ST_StartPoint(geom) AS pt FROM ds1_edges
    UNION ALL
    SELECT ST_EndPoint(geom)         FROM ds1_edges;
    CREATE INDEX ON ds1_edge_ends USING GIST (pt);

    DROP TABLE IF EXISTS tjunction_candidates;
    CREATE TEMP TABLE tjunction_candidates ON COMMIT DROP AS
    -- (a) unsnapped DS2 node near the interior of a DS1 edge
    SELECT
        nm.ds2_node_id                                    AS ds2_node_id,
        nm.ds2_node_id                                    AS split_node_id,
        nm.ds2_geom                                       AS ds2_geom,
        e.element_id                                      AS edge_id,
        e.geom                                            AS edge_geom,
        ST_SnapToGrid(ST_ClosestPoint(e.geom, nm.ds2_geom), coord_grid) AS split_geom,
        SQRT(POWER((ST_X(ST_ClosestPoint(e.geom, nm.ds2_geom)) - ST_X(nm.ds2_geom)) * m_per_deg_lon, 2) + POWER((ST_Y(ST_ClosestPoint(e.geom, nm.ds2_geom)) - ST_Y(nm.ds2_geom)) * m_per_deg_lat, 2))         AS dist_m,
        TRUE                                              AS moves_ds2_node
    FROM node_map nm
    LEFT JOIN ds2_node_groups g2 ON g2.element_id = nm.ds2_node_id
    JOIN edge_type_groups e
        ON  e.src = 'ds1'
        AND e.type_group <> 'road'
        AND ST_DWithin(e.geom, nm.ds2_geom, proximity_degrees)          -- GIST prefilter
    WHERE NOT nm.snapped
      AND SQRT(POWER((ST_X(ST_ClosestPoint(e.geom, nm.ds2_geom)) - ST_X(nm.ds2_geom)) * m_per_deg_lon, 2) + POWER((ST_Y(ST_ClosestPoint(e.geom, nm.ds2_geom)) - ST_Y(nm.ds2_geom)) * m_per_deg_lat, 2)) <= proximity
      AND (COALESCE(array_length(g2.groups, 1), 0) = 0
           OR e.type_group = 'other'
           OR e.type_group = ANY (g2.groups))

    UNION ALL

    -- (b) DS2 node snapped onto a DS1 interior vertex that joins no DS1 edge
    SELECT
        nm.ds2_node_id,
        nm.out_node_id,
        nm.ds2_geom,
        ...
        FALSE
    FROM node_map nm
    JOIN ds1_nodes d1 ON d1.element_id = nm.out_node_id
    JOIN edge_type_groups e
        ON  e.src = 'ds1'
        AND e.type_group <> 'road'
        AND ST_DWithin(e.geom, d1.geom, snap_tolerance)
    WHERE nm.snapped
      AND NOT EXISTS (SELECT 1 FROM ds1_edge_ends x WHERE ST_Equals(x.pt, d1.geom));

    DROP TABLE IF EXISTS tjunction_splits_raw;
    CREATE TEMP TABLE tjunction_splits_raw ON COMMIT DROP AS
    SELECT DISTINCT ON (c.split_node_id)
        c.dist_m,
        c.ds2_node_id, c.split_node_id, c.edge_id, c.split_geom, c.moves_ds2_node,
        ST_LineLocatePoint(c.edge_geom, c.split_geom)     AS fraction
    FROM tjunction_candidates c
    WHERE ST_LineLocatePoint(c.edge_geom, c.split_geom) > snap_tolerance
      AND ST_LineLocatePoint(c.edge_geom, c.split_geom) < 1.0 - snap_tolerance
      -- not already a road×crossing split point
      AND NOT EXISTS (SELECT 1 FROM ds1_edge_splits s WHERE s.node_id = c.split_node_id)
      -- at least one DS2 edge meets the DS1 edge at an angle of 30 deg or more
      AND EXISTS (
          SELECT 1
          FROM ds2_endpoint_dirs d,
               LATERAL (SELECT
                   ST_LineInterpolatePoint(c.edge_geom,
                       GREATEST(ST_LineLocatePoint(c.edge_geom, c.split_geom) - 1e-4, 0.0)) AS pa,
                   ST_LineInterpolatePoint(c.edge_geom,
                       LEAST(ST_LineLocatePoint(c.edge_geom, c.split_geom) + 1e-4, 1.0))    AS pb) loc
          WHERE ST_Equals(d.pt, c.ds2_geom)
            -- the edge must APPROACH the DS1 edge at this end: its far end is
            -- farther away. Otherwise an edge shorter than proximity would
            -- qualify at BOTH ends and split the DS1 edge twice at one point.
            AND SQRT(POWER((ST_X(ST_ClosestPoint(c.edge_geom, d.far)) - ST_X(d.far)) * m_per_deg_lon, 2) + POWER((ST_Y(ST_ClosestPoint(c.edge_geom, d.far)) - ST_Y(d.far)) * m_per_deg_lat, 2)) > c.dist_m
            AND ABS(  ((ST_X(loc.pb)-ST_X(loc.pa))*m_per_deg_lon) * ((ST_X(d.nxt)-ST_X(d.pt))*m_per_deg_lon)
                    + ((ST_Y(loc.pb)-ST_Y(loc.pa))*m_per_deg_lat) * ((ST_Y(d.nxt)-ST_Y(d.pt))*m_per_deg_lat) )
                < 0.866 * NULLIF(SQRT(POWER((ST_X(loc.pb) - ST_X(loc.pa)) * m_per_deg_lon, 2) + POWER((ST_Y(loc.pb) - ST_Y(loc.pa)) * m_per_deg_lat, 2)) * SQRT(POWER((ST_X(d.nxt) - ST_X(d.pt)) * m_per_deg_lon, 2) + POWER((ST_Y(d.nxt) - ST_Y(d.pt)) * m_per_deg_lat, 2)), 0)
      )
    ORDER BY c.split_node_id, c.dist_m;

    -- At most ONE node may split a DS1 edge at a given point; otherwise two node
    -- ids would share one coordinate and the two halves would not connect. The
    -- nearest candidate wins; any other stays where it is, unsplit.
    DROP TABLE IF EXISTS tjunction_splits;
    CREATE TEMP TABLE tjunction_splits ON COMMIT DROP AS
    SELECT DISTINCT ON (edge_id, ST_AsText(split_geom))
        ds2_node_id, split_node_id, edge_id, split_geom, moves_ds2_node, fraction
    FROM tjunction_splits_raw
    ORDER BY edge_id, ST_AsText(split_geom), dist_m, split_node_id;

    -- Split the DS1 edges at the junctions (downstream split machinery does the rest).
    INSERT INTO ds1_edge_splits (edge_id, node_id, split_geom, fraction)
    SELECT edge_id, split_node_id, split_geom, fraction FROM tjunction_splits;

    -- (a) The DS2 node moves onto the DS1 line so every edge meeting it agrees on
    -- one coordinate: DS2 sub-edges take it from node_map.out_geom.
    UPDATE node_map nm
    SET    out_geom = t.split_geom
    FROM   tjunction_splits t
    WHERE  t.moves_ds2_node AND nm.ds2_node_id = t.ds2_node_id;

    RAISE NOTICE 'Phase 3: T-junction splits of DS1 edges = %', (SELECT COUNT(*) FROM tjunction_splits);
    ...
    SELECT
        ...
        kv.key                                      AS prop_key,
        -- representative value for MERGE (last by ds2 id ordering), taken only
        -- from snaps whose attributes are eligible to merge (attr_merge_ok).
        -- NULL when no eligible snap supplied this key → nothing merged for it.
        (ARRAY_AGG(kv.value ORDER BY nm.ds2_node_id) FILTER (WHERE nm.attr_merge_ok))[
            array_upper(ARRAY_AGG(kv.value ORDER BY nm.ds2_node_id)
                        FILTER (WHERE nm.attr_merge_ok), 1)
        ]                                           AS prop_value,
        -- audit string: "<ds2_id>-<value>" joined across ALL snapped DS2 nodes,
        -- whether or not their attributes were merged — nothing is lost
        STRING_AGG(
    ...
    SELECT
        ds1_node_id,
        COALESCE(jsonb_object_agg(prop_key, prop_value)
                 FILTER (WHERE prop_value IS NOT NULL), '{}'::jsonb)
                                                    AS aggregated_ds2_props,
    ...
    --   proximity  = 1 - (snap_dist / proximity)   [metres; clamped 0..1]
    ...
    SELECT DISTINCT ON (out_node_id)
        ...
        GREATEST(0.0, LEAST(1.0, 1.0 - (snap_dist / NULLIF(proximity,0)))) AS proximity_score,
        COALESCE(type_match, 0.5)                           AS type_match_score,
        ROUND((
            0.7 * GREATEST(0.0, LEAST(1.0, 1.0 - (snap_dist / NULLIF(proximity,0))))
    ...
    -- Duplicate witnesses for Pass 1. Only filter-eligible DS1 edges qualify:
    -- an excluded DS1 edge passes through but cannot make a DS2 edge a duplicate.
    ...
    JOIN edge_type_groups etg
        ON etg.src = 'ds1' AND etg.element_id = e.edge_id AND etg.filter_ok
    ...
    WHERE u_out IS NOT NULL
      ...
      AND u_out != v_out
      -- Node-pair duplicate check applies only to filter-eligible DS2 edges.
      -- An excluded edge is kept even if its endpoints coincide with a DS1 pair.
      AND (
            NOT COALESCE(
                (SELECT det.filter_ok FROM edge_type_groups det
                 WHERE det.src = 'ds2' AND det.element_id = orig_edge_id), TRUE)
         OR NOT EXISTS (
    ...
    SELECT
        ...
        ST_Buffer(o.loc, edge_buffer_deg) AS buf
    FROM ds1_out o
    -- Only filter-eligible DS1 edges act as duplicate witnesses. An ineligible
    -- DS1 edge still passes through to the output; it simply cannot cause a DS2
    -- edge to be treated as its duplicate.
    JOIN edge_type_groups etg
        ON etg.src = 'ds1'
       AND etg.filter_ok
    ...
    SELECT
        ...
        -- DIRECTION: a duplicate runs ALONGSIDE its DS1 counterpart. Buffer
        -- coverage alone cannot tell "alongside" from "across": a short edge
        -- meeting a DS1 edge at a T sits wholly inside the buffer and measured
        -- as a 100% duplicate, so genuine connectors were deleted (Case G). An
        -- edge is treated as alongside when its chord is within 30 deg of the
        -- nearest same-type DS1 edge's local direction.
        COALESCE(al.along, TRUE) AS along
    FROM ds2_out d
    ...
    LEFT JOIN LATERAL (
        SELECT
            CASE
                WHEN SQRT(POWER((ST_X(ST_EndPoint(d.loc)) - ST_X(ST_StartPoint(d.loc))) * m_per_deg_lon, 2) + POWER((ST_Y(ST_EndPoint(d.loc)) - ST_Y(ST_StartPoint(d.loc))) * m_per_deg_lat, 2)) < 0.1 THEN TRUE  -- closed/degenerate chord
                ELSE ABS(  ((ST_X(q.pb)-ST_X(q.pa))*m_per_deg_lon)
                             * ((ST_X(ST_EndPoint(d.loc))-ST_X(ST_StartPoint(d.loc)))*m_per_deg_lon)
                         + ((ST_Y(q.pb)-ST_Y(q.pa))*m_per_deg_lat)
                             * ((ST_Y(ST_EndPoint(d.loc))-ST_Y(ST_StartPoint(d.loc)))*m_per_deg_lat) )
                     >= 0.866 * SQRT(POWER((ST_X(q.pb) - ST_X(q.pa)) * m_per_deg_lon, 2) + POWER((ST_Y(q.pb) - ST_Y(q.pa)) * m_per_deg_lat, 2)) * SQRT(POWER((ST_X(ST_EndPoint(d.loc)) - ST_X(ST_StartPoint(d.loc))) * m_per_deg_lon, 2) + POWER((ST_Y(ST_EndPoint(d.loc)) - ST_Y(ST_StartPoint(d.loc))) * m_per_deg_lat, 2))
            END AS along
        FROM (
            SELECT o.loc,
                   ST_LineLocatePoint(o.loc, ST_LineInterpolatePoint(d.loc, 0.5)) AS f
            FROM ds1_out o
            JOIN edge_type_groups ot
                ON ot.src = 'ds1' AND ot.element_id = o.orig_edge_id
            WHERE ot.type_group = COALESCE(dt.type_group, 'other')
              AND ST_DWithin(o.loc, d.loc, edge_buffer_deg)             -- same corridor as coverage
            ORDER BY o.loc <-> ST_LineInterpolatePoint(d.loc, 0.5)      -- KNN
            LIMIT 1
        ) nb,
        LATERAL (SELECT ST_LineInterpolatePoint(nb.loc, GREATEST(nb.f - 1e-4, 0.0)) AS pa,
                        ST_LineInterpolatePoint(nb.loc, LEAST(nb.f + 1e-4, 1.0))    AS pb) q
    ) al ON TRUE;
    ...
    -- ── Which DS2 edges survive duplicate removal ────────────────────────────
    -- Base survivors: an edge is kept when it is filter-exempt, is not mostly
    -- covered, does not run alongside its DS1 counterpart, or joins two nodes
    -- already on the DS1 network (the connectivity guard).
    DROP TABLE IF EXISTS ds2_keep;
    CREATE TEMP TABLE ds2_keep ON COMMIT DROP AS
    SELECT d.sub_edge_id, d.u_out, d.v_out
    FROM ds2_out d
    JOIN ds2_coverage_ratio r ON r.sub_edge_id = d.sub_edge_id
    LEFT JOIN edge_type_groups det
        ON det.src = 'ds2' AND det.element_id = d.orig_edge_id
    WHERE NOT COALESCE(det.filter_ok, TRUE)
       OR r.covered_pct < edge_overlap
       OR NOT r.along
       OR (    EXISTS (SELECT 1 FROM shared_out_nodes s WHERE s.node_id = d.u_out)
           AND EXISTS (SELECT 1 FROM shared_out_nodes s WHERE s.node_id = d.v_out));
    CREATE INDEX ON ds2_keep (sub_edge_id);
    CREATE INDEX ON ds2_keep (u_out);
    CREATE INDEX ON ds2_keep (v_out);

    -- STRANDING GUARD (routability). Removing a duplicate is only safe where its
    -- endpoints are on the DS1 network — the DS1 edge then carries the
    -- connection instead. If an endpoint is NOT on the DS1 network and a
    -- surviving edge uses it, removing the duplicate would strand that edge, so
    -- the duplicate is kept. Repeated until stable, so protection propagates
    -- along a chain of duplicates rather than stopping one segment short.
    LOOP
        INSERT INTO ds2_keep (sub_edge_id, u_out, v_out)
        SELECT d.sub_edge_id, d.u_out, d.v_out
        FROM ds2_out d
        WHERE NOT EXISTS (SELECT 1 FROM ds2_keep k WHERE k.sub_edge_id = d.sub_edge_id)
          AND (
                (    NOT EXISTS (SELECT 1 FROM shared_out_nodes s WHERE s.node_id = d.u_out)
                 AND EXISTS (SELECT 1 FROM ds2_keep k WHERE k.u_out = d.u_out OR k.v_out = d.u_out))
             OR (    NOT EXISTS (SELECT 1 FROM shared_out_nodes s WHERE s.node_id = d.v_out)
                 AND EXISTS (SELECT 1 FROM ds2_keep k WHERE k.u_out = d.v_out OR k.v_out = d.v_out))
          );
        GET DIAGNOSTICS keep_added = ROW_COUNT;
        EXIT WHEN keep_added = 0;
    END LOOP;
    ...
    SELECT d.*
    FROM ds2_out d
    WHERE EXISTS (SELECT 1 FROM ds2_keep k WHERE k.sub_edge_id = d.sub_edge_id);
    ...
    JOIN zone_polygons p2
        ...
        -- ATTRIBUTE FILTER: both sides must be eligible. Zone attributes live on
        -- the source zone rows, so eligibility is checked there. An ineligible
        -- zone matches nothing and is carried through / added as new.
        AND (f_zone IS NULL OR EXISTS (
                SELECT 1 FROM testzones tz, jsonb_array_elements(f_zone) g
                WHERE tz.source = p1.source AND tz.element_id = p1.element_id
                  AND (tz.feature::jsonb->'properties') @> g))
        AND (f_zone IS NULL OR EXISTS (
                SELECT 1 FROM testzones tz, jsonb_array_elements(f_zone) g
                WHERE tz.source = p2.source AND tz.element_id = p2.element_id
                  AND (tz.feature::jsonb->'properties') @> g))
    ...
    -- ── Orphan node cleanup ─────────────────────────────────────────────────
    -- A DS2 node that did not snap is emitted as a new output node. If its edge
    -- is later removed as a duplicate, that node is left referenced by nothing
    -- and appears in the output as a stray point on the map.
    --
    -- Drop any output node that no surviving output EDGE and no surviving
    -- output ZONE refers to. Zones reference nodes through _w_id, so they must
    -- be consulted too or zone rings would lose their vertices.
    --
    -- Nodes exist to define network topology, so one that nothing references
    -- carries no meaning; extension points are a separate file type and are
    -- unaffected by this.
    DROP TABLE IF EXISTS referenced_node_ids;
    CREATE TEMP TABLE referenced_node_ids ON COMMIT DROP AS
    SELECT DISTINCT (feature::jsonb->'properties'->>'_u_id') AS id FROM new_export_edges
    UNION
    SELECT DISTINCT (feature::jsonb->'properties'->>'_v_id')      FROM new_export_edges
    UNION
    SELECT DISTINCT jsonb_array_elements_text(
               COALESCE(feature::jsonb->'properties'->'_w_id', '[]'::jsonb))
    FROM new_export_zones;

    CREATE INDEX ON referenced_node_ids (id);

    DELETE FROM new_export_nodes nen
    WHERE NOT EXISTS (
        SELECT 1 FROM referenced_node_ids r WHERE r.id = nen.id::TEXT
    );

    RAISE NOTICE 'Phase 7: orphan node cleanup — % nodes remain', 
    ...
    JOIN ext_points p1
        ...
        AND ST_DWithin(p2.geom, p1.geom, proximity_degrees)          -- indexed prefilter
        AND SQRT(POWER((ST_X(p2.geom) - ST_X(p1.geom)) * m_per_deg_lon, 2) + POWER((ST_Y(p2.geom) - ST_Y(p1.geom)) * m_per_deg_lat, 2)) <= proximity   -- exact circle
        -- ATTRIBUTE FILTER: both sides must be eligible. A point failing the
        -- filter finds no match and is carried through as a new point.
        AND (f_point IS NULL OR EXISTS (
                SELECT 1 FROM jsonb_array_elements(f_point) g
                WHERE (p1.feature::jsonb->'properties') @> g))
        AND (f_point IS NULL OR EXISTS (
                SELECT 1 FROM jsonb_array_elements(f_point) g
                WHERE (p2.feature::jsonb->'properties') @> g))
    ...
    SELECT
        ...
        geom,
        feature,
        ST_Buffer(geom, line_buffer_deg) AS buf
    FROM ext_lines
    WHERE source = src_one_tdei_dataset_id
      -- Only filter-eligible DS1 lines act as merge witnesses; ineligible ones
      -- still pass through to the output.
      AND (f_line IS NULL OR EXISTS (
              SELECT 1 FROM jsonb_array_elements(f_line) g
              WHERE (feature::jsonb->'properties') @> g));
    ...
    FROM (
        ...
        JOIN ds1_line_buffers b
            ...
            -- ATTRIBUTE FILTER: the DS2 line must be eligible (the DS1 side is
            -- filtered when ds1_line_buffers is built). An ineligible line finds
            -- no match and is carried through as a new line.
            AND (f_line IS NULL OR EXISTS (
                    SELECT 1 FROM jsonb_array_elements(f_line) g
                    WHERE (l2.feature::jsonb->'properties') @> g))
    ) candidates
    WHERE overlap_ratio >= line_overlap
    ...
    JOIN poly_valid p2
        ...
        -- ATTRIBUTE FILTER: both sides must be eligible. An ineligible polygon
        -- finds no pair and is carried through unchanged / added as new.
        AND (f_polygon IS NULL OR EXISTS (
                SELECT 1 FROM jsonb_array_elements(f_polygon) g
                WHERE (p1.feature::jsonb->'properties') @> g))
        AND (f_polygon IS NULL OR EXISTS (
                SELECT 1 FROM jsonb_array_elements(f_polygon) g
                WHERE (p2.feature::jsonb->'properties') @> g))
    ...
    FROM poly_pairs pp
    WHERE pp.overlap_ratio > polygon_overlap;
...
ALTER FUNCTION content.tdei_union_dataset(CHARACTER VARYING, CHARACTER VARYING, REAL, JSONB)
    OWNER TO tdeiadmin;