...
export class UploadOswCompressionResponseHandler extends WorkflowHandlerBase {

    constructor(workflowEvent: EventEmitter, orchestratorServiceInstance: IOrchestratorService) {
        super(workflowEvent, orchestratorServiceInstance, "OSW_UPLOAD_COMPRESSION_RESPONSE_HANDLER");
    }
    ...
    async handleRequest(message: QueueMessage, delegate_worflow: string[], params: any): Promise<void> {
        ...
        try{
            if(message.data.success){
                ...
                const isOsm = message.data.isOsm;

                console.log('zip path ', zip_path);
                if(isOsm){  
                    await dbClient.query(DatasetEntity.getUpdateDatasetOsmZipUrlQuery(job.response_props.tdei_dataset_id, zip_path));
                    console.log('Updated OSM zip url');
                    return;
                }
                else {
