...
class PathwaysService implements IPathwaysService {
    ...
    async processPublishRequest(user_id: string, tdei_dataset_id: string): Promise<string> {
        ...
        const metadata = await this.tdeiCoreServiceInstance.getMetadataDetailsById(tdei_dataset_id);
        ...
        const queryResult = await dbClient.query(metadata.getOverlapQuery(TDEIDataType.pathways, dataset.tdei_project_group_id, dataset.tdei_service_id));
        ...
        const workflow_identifier = "PATHWAYS_PUBLISH_VALIDATION_REQUEST_WORKFLOW";
        ...
        const trigger_workflow = appContext.orchestratorServiceInstance?.getWorkflowByIdentifier(workflow_identifier);
    ...
    async processValidationOnlyRequest(user_id: string, datasetFile: any): Promise<string> {
        ...
        const job_id = await this.jobServiceInstance.createJob(job);
        //Compose the meessage
        const workflow_identifier = "PATHWAYS_VALIDATION_ONLY_VALIDATION_REQUEST_WORKFLOW";
        const queueMessage = QueueMessage.from({
            messageId: job_id.toString(),
            messageType: workflow_identifier,
            data: {
    ...
    async processUploadRequest(uploadRequestObject: IUploadRequest): Promise<string> {
        ...
        let changesetUploadUrl: string | undefined = "";
        if (uploadRequestObject.changesetFile) {
            const changesetStorageFilePath = path.join(storageFolderPath, 'changeset.txt');
            changesetUploadUrl = await storageService.uploadFile(changesetStorageFilePath, 'text/plain', Readable.from(uploadRequestObject.changesetFile[0].buffer), "gtfspathways") as string;
        }
        ...
        datasetEntity.changeset_url = decodeURIComponent(changesetUploadUrl ?? "");
        ...
        //Compose the meessage
        const workflow_identifier = "PATHWAYS_UPLOAD_VALIDATION_REQUEST_WORKFLOW";
        const queueMessage = QueueMessage.from({
            messageId: job_id.toString(),
            messageType: workflow_identifier,
            data: {
